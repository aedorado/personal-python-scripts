...
ssrss_albums = ['72157693711948081', '72157690449465555', '72157719250166952', '72157677029564726',
               '72157674042736540', '72157666606335405', '72157660846200719', '72157660617883467', '72157671465198206']
...
def write_images():
   headers = ["id", "album_id", "tags", "display_url", "download_url",
              "thumbnail_url", "type", "date", "color", "location", "description"]
   ...
   for album in all_albums:
       ...
       elif album not in ssrss_albums:
           print(f'Adding images for the album {album}')
           filename = 'files/maharaja_images.csv'
           ImageRowWriter(album, filename, headers).start()
   ...
   def run(self):  # write_rows_to_images_csv
       ...
       for photo in photos:
           ...
           try:
               ...
               total_row_processed = total_row_processed + 1
               if (total_row_processed % 50 == 0):
                   print(
                       f'Processed {total_row_processed} rows. {round(total_row_processed / len(photos) * 100, 2)}%')
           except Exception:
               print(
...
def write_row_to_album_csv(album_filename, headers, albums, is_ssrss):
   with open(album_filename, 'a', newline='') as csvfile:
       ...
       for album in albums:
           ...
           elif not is_ssrss and not ssrss_albums.__contains__(album['id']):
               writer.writerow(album_to_write)
               all_albums.append(album['id'])
